...
const customSliders = {
    slickSliderHero() {
        ...
        const getAnimationItems = function () {
            ...
            var allInactiveSlideAllElements = [];
            const allSlides = document.querySelectorAll('.slick-slide');
            console.log(allSlides);
            for (let i = 0; i < allSlides.length; i++) {
                const slide = $(allSlides[i]);
                console.log(slide);
                if (slide[0] === $('.slick-slide.slick-current.slick-active')[0]) {
                    activeSlide = slide;
                }
                else {
                    allInactiveSlideAllElements.push(slide.find('.slideTitle'));
                    allInactiveSlideAllElements.push(slide.find('.slideDescription'));
                    allInactiveSlideAllElements.push(slide.find('.slideSubTitle'));
                    allInactiveSlideAllElements.push(slide.find('.stackItem'));
                    allInactiveSlideAllElements.push(slide.find('.backgroundImage'));
                    allInactiveSlideAllElements.push(slide.find('.slideCtaWrapper'));
                    allInactiveSlideAllElements.push(slide.find('.imageStackSlider'));
                    allInactiveSlideAllElements.push(slide.find('.slideLayer'));
                }
            }
        ...
        const showInactiveElements = function () {
            const localAnimationItems = getAnimationItems();
            for (let index = 0; index < localAnimationItems.allInactiveSlideAllElements.length; index++) {
                const list = localAnimationItems.allInactiveSlideAllElements[index];
                console.log('inactieve lijst to show', list);
                for (let elementIndex = 0; elementIndex < list.length; elementIndex++) {
                    const element = list[elementIndex];
                    console.log('inactief element to show', element);
                    $(element).fadeTo(0, 1);
                }
            }
        };

        const hideInactiveElements = function () {
            const localAnimationItems = getAnimationItems();
            for (let index = 0; index < localAnimationItems.allInactiveSlideAllElements.length; index++) {
                const list = localAnimationItems.allInactiveSlideAllElements[index];
                console.log('inactieve lijst to hide', list);
                for (let elementIndex = 0; elementIndex < list.length; elementIndex++) {
                    const element = list[elementIndex];
                    console.log('inactief element to hide', element);
                    $(element).fadeTo(0, 0);
                    // element.hide();
                }
            }
